...
class Operators:
    ...
    def __init__(self, operator_list, device):
        ...
        self.constant_operators = [x for x in operator_list if x.replace('.', '').strip('-').isnumeric()]  # 小数
        self.nonvar_operators = [x for x in self.operator_list if "var_" not in x and x not in self.constant_operators]  # 非变量非常数的运算符
        self.var_operators = [x for x in operator_list if x not in self.nonvar_operators and x not in self.constant_operators]  # 变量
        ...
        self.arity_two = torch.tensor([i for i in range(len(self.operator_list)) if self.arity_dict[self.operator_list[i]] == 2], dtype=torch.int)
        # Contains indices of all operators with arity 1
        self.arity_one = torch.tensor([i for i in range(len(self.operator_list)) if self.arity_dict[self.operator_list[i]] == 1], dtype=torch.int)
        # Contains indices of all operators with arity 0
        self.arity_zero = torch.tensor([i for i in range(len(self.operator_list)) if self.arity_dict[self.operator_list[i]] == 0], dtype=torch.int)
        # Contains indices of all operators that are variables
        self.variable_tensor = torch.tensor([i for i in range(len(self.operator_list)) if operator_list[i] in self.var_operators], dtype=torch.int)
        ...
        # 自定义三角函数和对指数约束mask
        self.non_sin_cos_mask = torch.Tensor([0 if i in ['sin', 'cos'] else 1 for i in self.operator_list])  # sin cos的位置为0其他位置为1
        self.non_exp_ln_mask = torch.Tensor([0 if i in ['ln', 'exp'] else 1 for i in self.operator_list])  # log exp的位置为0其他位置为1
        self.cos_sin_index = torch.Tensor([i for i in range(len(self.operator_list)) if self.operator_list[i] in ['sin', 'cos']])
        self.exp_ln_index = torch.Tensor([i for i in range(len(self.operator_list)) if self.operator_list[i] in ['exp', 'ln']])
        ...
        self.float_tokens = np.array([i for i, f in enumerate(self.operator_list) if f.replace('.', '').strip('-').isnumeric()])  # 小数
